...
function startapp_scripts() {
	...
	if ( ( $is_blog || $is_shop ) && ! wp_script_is( 'isotope' ) ) {
		wp_enqueue_script( 'isotope', STARTAPP_TEMPLATE_URI . '/js/vendor/isotope.pkgd.min.js', array( 'jquery' ), null, true );
	}
...
function startapp_tgm_init() {
	$dir = wp_normalize_path( STARTAPP_TEMPLATE_DIR . '/plugins' );

	$plugins = array(
		array(
			'name'     => esc_html__( 'StartApp Core', 'startapp' ),
			'slug'     => 'startapp-core',
			'source'   => $dir . '/startapp-core.zip',
			'version'  => '1.1.1',
			'required' => true,
		),
		array(
			'name'     => esc_html__( 'Visual Composer', 'startapp' ),
			'slug'     => 'js_composer',
