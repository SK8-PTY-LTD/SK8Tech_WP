...
<?php if ( $has_orders ) : ?>
	...
	<div class="table-responsive">
		<table class="woocommerce-MyAccount-orders shop_table my_account_orders account-orders-table">
			<thead>
			<tr>
						...
						<td class="<?php echo esc_attr( $column_id ); ?>"
							...
							<?php elseif ( 'order-number' === $column_id ) : ?>
								<a href="<?php echo esc_url( $order->get_view_order_url() ); ?>">
									<?php echo _x( '#', 'hash before order number', 'startapp' ) . $order->get_order_number(); ?>
								</a>
							...
							<?php elseif ( 'order-total' === $column_id ) : ?>
								...
								<?php
								echo sprintf( _n( '%s for %s item', '%s for %s items', $item_count, 'startapp' ),
							...
							<?php elseif ( 'order-actions' === $column_id ) : ?>
								...
								$actions = array(
									'pay'    => array(
										...
										'name' => esc_html__( 'Pay', 'startapp' ),
									),
									'view'   => array(
										'url'  => $order->get_view_order_url(),
										'name' => esc_html__( 'View', 'startapp' ),
									),
									'cancel' => array(
										'url'  => $order->get_cancel_order_url( wc_get_page_permalink( 'myaccount' ) ),
										'name' => esc_html__( 'Cancel', 'startapp' ),
									),
								);
		...
		<div class="woocommerce-Pagination">
			<?php if ( 1 !== $current_page ) : ?>
				<a class="woocommerce-Button woocommerce-Button--previous button" href="<?php echo esc_url( wc_get_endpoint_url( 'orders', $current_page - 1 ) ); ?>">
					<?php esc_html_e( 'Previous', 'startapp' ); ?>
				</a>
			<?php endif; ?>
...
<?php else : ?>
	<div class="woocommerce-Message woocommerce-Message--info woocommerce-info">
		<a class="woocommerce-Button button" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
			<?php esc_html_e( 'Go Shop', 'startapp' ) ?>
		</a>
		<?php esc_html_e( 'No order has been made yet.', 'startapp' ); ?>
	</div>
<?php endif; ?>
