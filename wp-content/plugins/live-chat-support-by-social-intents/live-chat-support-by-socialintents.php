...
Description: Delight visitors, improve service, and sell more with Live Chat. To get started: 1) Click the "Activate" link to the left of this description, 2) Go to your Live Chat configuration page, and register for a new account.
Version: 1.1.33
...
function silc_notice() {
    if(!get_option('silc_widgetID')) echo('<div class="error"><p><strong>'.sprintf(__('Your Live Chat Support Plugin is disabled. Please go to the <a href="%s">plugin settings</a> to register.  ' ), admin_url('options-general.php?page=live-chat-support-by-socialintents')).'</strong></p></div>');
}
...
<div class="postbox" style="float:left;width:35em">
            <h3 class="hndle"><span id="silc_noAccountSpan"><?php _e('Live Chat Free Registration', $silc_domain) ?></span></h3>
...
<p style="text-align:center"><?php wp_nonce_field('update-options') ?>
		...
		<p><?php printf(__('Join 25,000+ companies using our tools to improve customer service and sell more. Visit %1$sSocial Intents%2$ssocialintents.com%3$s to 
				see how we can help you.', $silc_domain), '<a href="
http://www.socialintents.com/live-chat.html" target="_blank" title="', '">', '</a>') ?></p>
        ...
        <div  id="addAppKey" class="postbox" style="float:left;width:35em;margin-right:10px">
            ...
            <div class="inside" style="padding: 0 10px">
                <form id="saveSettings" method="post" action="options.php">
                    ...
                    <p><label for="silc_widgetID"><?php printf(__('Enter your App Key below to activate your plugin.  <br><br> If you\'ve already signed up, <a href=\'http://www.socialintents.com/login.do\' target=\'_blank\'>login here</a> to grab your key under Apps, select Live Chat, then Edit Settings.  Your Key is under the "Your App Key" tab.<br>', $silc_domain), '<strong><a href="http://www.socialintents.com/" title="', '">', '</a></strong>') ?></label><br />
			<input type="text" name="silc_widgetID" id="silc_widgetID" placeholder="Your app Key" value="<?php echo(get_option('silc_widgetID')) ?>" style="width:100%" />
