...
if ( ! function_exists( 'equip_get_html_attr' ) ) :
	/**
	 ...
	 * @return string
	 */
	function equip_get_html_attr( $attr ) {
		if ( empty( $attr ) ) {
			return '';
		}
...
if ( ! function_exists( 'equip_get_icons' ) ) :
	...
	function equip_get_icons( $pack ) {
		 ...
		 *
		 * For more information about the format of an array see
		 * @link http://codeb.it/fonticonpicker/
		 *
		 * @param array $icons A list of icons
		 */
		return apply_filters( "equip/icons/{$pack}", [] );
	}
endif;
		...
		switch ( $type ) {
			case 'paired':
				printf( '<%1$s %2$s>%3$s</%1$s>',
					$tag,
					equip_get_html_attr( $attributes ),
					$contents
				);
				break;
			...
			case 'self-closing':
			default:
				printf( '<%1$s %2$s>', $tag, equip_get_html_attr( $attributes ) );
				break;
		}
	}
endif;
...
if ( ! function_exists( 'equip_get_tag' ) ) :
	...
	function equip_get_tag( $tag, $attributes = [], $contents = '', $type = 'self-closing' ) {
		...
		switch ( $type ) {
			...
			case 'self-closing':
			default:
				$result = sprintf( '<%1$s %2$s>', $tag, equip_get_html_attr( $attributes ) );
				break;
		}
