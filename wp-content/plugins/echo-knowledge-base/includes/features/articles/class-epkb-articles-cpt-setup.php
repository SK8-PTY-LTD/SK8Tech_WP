...
class EPKB_Articles_CPT_Setup {
	...
	public function __construct() {
		...
		add_filter( 'rewrite_rules_array', array( $this, 'update_article_slug_rewrite_rules' ) );
	...
	public static function register_custom_post_type( $kb_id, $current_id, $kb_articles_common_path = '' ) {
		...
		$show_post_in_ui = ( $kb_id == $current_id );
		...
		/* we do not have tax taxonomy right now
	...
	/**
	 * Ensure that layouts that need rewrite rule to handle articles on the same page get loaded on rules being flushed
	 *
	 * @param array $rules
	 * @return array
	 */
	function update_article_slug_rewrite_rules( $rules ) {
		foreach ( epkb_get_instance()->kb_config_ojb->get_kb_configs() as $kb_config ) {

			// if Elegant Layouts is being deactivated then don't add rewrite rule below
			if ( defined('ELEG_LAY_REMOVE_REWRITE_RULE') ) {
				continue;
			}

			if ( EPKB_KB_Config_Layouts::is_main_page_displaying_sidebar( $kb_config['kb_main_page_layout'] ) ||
			     EPKB_KB_Config_Layouts::is_article_page_displaying_sidebar( $kb_config['kb_article_page_layout'] ) ) {
				$rules = array_merge( array( $kb_config['kb_articles_common_path'] . '/([^/]+)/?$' =>
					                             'index.php?pagename=' . urlencode( $kb_config['kb_articles_common_path'] ) . '&epkb_article_page=$matches[1]' ), $rules );
			}
		}
