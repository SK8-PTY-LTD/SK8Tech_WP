...
class EPKB_KB_Handler {
	...
	public static function get_tag_taxonomy_name( $kb_id ) {
		return self::get_post_type( $kb_id ) . '_tag';
	}
	 ...
	 * @return int|null if current page is KB main page otherwise null
	 */
	public static function get_kb_id_from_kb_main_shortcode( $the_post=null ) {
		...
		add_shortcode( self::KB_MAIN_PAGE_SHORTCODE_NAME, array( 'EPKB_Layouts_Setup', 'output_kb_main_page_shortcode' ) );

		$post = empty($the_post) ? $post : $the_post;

		// determine whether this page contains this plugin shortcode
		$content = '';
		if ( $post != null && has_shortcode( $post->post_content, self::KB_MAIN_PAGE_SHORTCODE_NAME ) ) {
			$content = $post->post_content;
		} else if ( isset($post->ID) ) {
			$content = $wpdb->get_var( "SELECT meta_value FROM $wpdb->postmeta " .
			                           "WHERE post_id = {$post->ID} and meta_value LIKE '%%" . self::KB_MAIN_PAGE_SHORTCODE_NAME . "%%'" );
		}
	...
	public static function get_kb_main_pages( $kb_config ) {
		...
		foreach ( $kb_main_pages as $post_id => $post_title ) {
			...
			if ( empty( $post_status ) || $post_status == 'inherit' ) {
			...
			$kb_main_pages_info[$post_id] = array( 'post_title' => $post_title, 'post_status' => $post_status, 'post_slug' => $post->post_name );
		}
	...
	/**
	 * Determines if current page has URL with article slug e.g.  /my-main-page/article-123
	 *
	 * @return bool
	 */
	public static function is_on_page_with_article_url() {
		$kb_article_var = get_query_var( 'epkb_article_page', '');
		return ! empty($kb_article_var);
	}
}
