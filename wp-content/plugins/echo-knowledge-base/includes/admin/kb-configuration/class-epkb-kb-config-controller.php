...
class EPKB_KB_Config_Controller {
	...
	public function change_one_configuration_param() {
		...
		if ( empty($_REQUEST['epkb_is_article_icon_active']) || $_REQUEST['epkb_is_article_icon_active'] != "true" ) {
			...
			$output = $kb_config_page->display_article_page_layout_preview( false );
		}
	...
	public function change_main_page_config_ajax() {
		...
		$article_output = array('article_page_output' => '', 'article_widget' => '', 'article_style' => '', 'article_colors' => '', 'article_text' => '');
		$style_tab_output = 'NONE';
		$colors_tab_output = 'NONE';
		$article_layout_widget = 'NONE';
		$message = '';
		$common_path_output = '';
		$overview_output = '';
		$ordering_output = '';
		$main_page_text_output = '';
		$kb_config_page = new EPKB_KB_Config_Page( $kb_config );

		if ( $target_type == 'layout' ) {

			// get Ordering tab
			$ordering_output = $kb_config_page->get_order_sidebar_form();
			...
			$main_page_text_output = $kb_config_page->get_text_sidebar_form();

			// udpate Article Page to default layout, style and colors etc.
			$default_article_page_layout = $chosen_main_page_layout == 'Grid' ? EPKB_KB_Config_Layouts::SIDEBAR_LAYOUT :
																			    EPKB_KB_Config_Layouts::KB_ARTICLE_PAGE_NO_LAYOUT;
			$kb_config['kb_article_page_layout'] = $default_article_page_layout;

			// get article output
			$kb_config_page = new EPKB_KB_Config_Page( $kb_config );
			$article_layout_widget = $kb_config_page->get_article_page_layout_sidebar_form();
			$article_output = $this->generate_article_layout_and_sidebar( $target_type, $target_name, $kb_config, $default_article_page_layout );

			// get common path widget - based on both Main and Article page layout
			ob_start();
			$common_path = isset($kb_config['kb_articles_common_path']) ? $kb_config['kb_articles_common_path'] : '';  // TODO remove isset() after 1.0.1.
			$kb_config_page->display_articles_common_path( $target_name, $common_path );
			$common_path_output = ob_get_clean();
		}
		...
		if ( $target_type == 'layout' || $target_type == 'style' ) {
			...
			$kb_config_page = new EPKB_KB_Config_Page( $kb_config );
			$style_tab_output = $kb_config_page->get_styles_sidebar_form();
		...
		if ( $target_type == 'layout' ||  $target_type == 'colors' ) {
			...
			$kb_config_page = new EPKB_KB_Config_Page( $kb_config );
			$colors_tab_output = $kb_config_page->get_colors_sidebar_form();
		...
		// we are done here
		wp_die( json_encode( array( 'kb_main_page_output' => $main_page_layout, 'overview_output' => $overview_output, 'common_path_output' => $common_path_output,
		                            'ordering_output' => $ordering_output, 'style_tab_output' => $style_tab_output, 'colors_tab_output' => $colors_tab_output,
									'article_page_output' => $article_output['article_page_output'], 'kb_article_layout_widget' => $article_layout_widget,
									'article_style_tab_output' => $article_output['article_style'], 'article_colors_tab_output' => $article_output['article_colors'],
									'article_text_tab_output' => $article_output['article_text'], 'main_page_text_output' => $main_page_text_output,
									'article_page_layout' => $kb_config['kb_article_page_layout'],
	...
	private function generate_article_layout_and_sidebar( $target_type, $target_name, $kb_config, $chosen_article_page_layout ) {
		...
		if ( $target_type == 'layout' ) {
			$kb_config_page = new EPKB_KB_Config_Page( $kb_config );
			$text_tab_output = $kb_config_page->get_article_text_sidebar_form();
		}
		...
		if ( $target_type == 'layout' || $target_type == 'style' ) {
			...
			$kb_config = array_merge($kb_config, $reset_style_config);
			$kb_config_page = new EPKB_KB_Config_Page( $kb_config );
			$style_tab_output = $kb_config_page->get_article_styles_sidebar_form();
			$message .= __( 'Style was set to', 'echo-knowledge-base' ) . ' ' . ucfirst($target_name) . '. ' ;
		}
		...
		if ( $target_type == 'layout' ||  $target_type == 'colors' ) {
			...
			$kb_config = array_merge($kb_config, $reset_color_config);
			$kb_config_page = new EPKB_KB_Config_Page( $kb_config );
			$colors_tab_output = $kb_config_page->get_article_colors_sidebar_form();
			$message = $this->get_color_change_msg( $target_name );
		}
		...
		// update Article Page layout
		$kb_config_page = new EPKB_KB_Config_Page( $kb_config );
		$article_page_output = $kb_config_page->display_article_page_layout_preview( false );

		$message .= __( 'Configuration NOT saved.', 'echo-knowledge-base' );

		return array( 'article_page_output' => $article_page_output, 'article_text' => $text_tab_output,
		              'article_style' => $style_tab_output, 'article_colors' => $colors_tab_output, 'message' => $message );
	}
	...
	public function save_kb_config_changes_in_db() {
		...
		// Articles common path needs extra processing
		...
		// set articles common path based on layout and user selection
		...
		$is_layout_with_article = EPKB_KB_Config_Layouts::is_main_page_displaying_sidebar( $new_kb_config['kb_main_page_layout']  )
		                          || EPKB_KB_Config_Layouts::is_article_page_displaying_sidebar( $new_kb_config['kb_article_page_layout'] );

		// if user manage to setup custom slug (shouldn't) then use first KB Main Page slug instead
		if ( $is_layout_with_article && $kb_articles_common_path_rbtn == 'path_custom_slug' ) {
			$kb_main_pages_info = EPKB_KB_Handler::get_kb_main_pages( $new_kb_config );
			$new_kb_config['kb_articles_common_path'] = '';
			foreach ( $kb_main_pages_info as $post_id => $post_info ) {
				$new_kb_config['kb_articles_common_path'] = $post_info['post_slug'];
				break;
			}
		} else if ( ! empty($kb_articles_common_path_rbtn) && $kb_articles_common_path_rbtn != 'path_custom_slug' ) {
			$new_kb_config['kb_articles_common_path'] = $kb_articles_common_path_rbtn;
		}
		$new_kb_config['kb_articles_common_path'] = sanitize_title_with_dashes( $new_kb_config['kb_articles_common_path'], '', 'save' );
