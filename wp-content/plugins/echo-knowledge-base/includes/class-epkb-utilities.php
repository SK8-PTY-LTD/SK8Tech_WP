...
class EPKB_Utilities {
	...
	/**
	 * Retrieve CPT post object given post slug.
	 *
	 * @param $post_slug
	 * @param $kb_id
	 * @return null|WP_Post - return null if post cannot be returned
	 */
	public static function get_cpt_post_from_slug( $post_slug, $kb_id ) {

		// retrieve the post and ensure it is one
		$post = get_page_by_path( $post_slug, OBJECT, EPKB_KB_Handler::get_post_type( $kb_id ) );
		if ( empty($post) || ! is_object($post) || ! $post instanceof WP_Post ) {
			return null;
		}
	 ...
	 *
	 * WARN: Use ep.kb_get_instance()->kb_config_ojb->get_kb_configs() to get KB specific configuration.
	 *
