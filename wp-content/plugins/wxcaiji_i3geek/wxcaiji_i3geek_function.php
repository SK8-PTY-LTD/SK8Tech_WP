...
define("I3GEEK_READING_PLUGIN_ROOT",ABSPATH."wp-content/plugins/wxcaiji_i3geek/");
...
class wxcaiji_i3geek_function{
	...
	function down2file($url,$site_url){
		...
		$title = $myhtml->find ('h2[id=activity-name]',0 )->plaintext; 
		$time = $myhtml->find ('em[id=post-date]',0 )->plaintext; 
		$content = $myhtml->find ('div.rich_media_content',0 )->innertext; 
		$myhtml = str_get_html($content);
		$ret = $myhtml->find('img');
